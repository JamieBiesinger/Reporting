        ...
        (
          (SELECT DISTINCT
           ...
           ,CASE WHEN NOT ( a.QuickestApproval IS NULL )
                 THEN CASE WHEN sr.region = 'Montana'
...
DISTINCT
               ...
               ,CASE WHEN NOT ( import.CornealPreservationOn IS NULL )
                          AND et.RecoveryIntent = 'Transplant'
                     THEN DATEDIFF(mi, import.deathon,
                                   import.CornealPreservationOn)
                     ELSE NULL
                END AS DtoPminutes
            ...
            FROM
                ( Warehouse.itxPlus.ImportTissueParser import
                  LEFT OUTER JOIN Warehouse.dbo.CorneaTissueHeirarchy ch ON import.TissueIdentifier = ch.TissueIdentifier
                  LEFT OUTER JOIN (
                                    SELECT
                                        MAX(ship.ShipDateX) AS max_shipdate
                                       ,a.TissueIdentifier
                                    FROM
                                        SIGHTLIFE.Data.ShipmentRequestInventory
                                        AS a
                                        LEFT OUTER JOIN warehouse.itxplus.ShipX
                                        AS ship ON ship.Id = a.ShipmentId
                                    WHERE
                                        ( a.ShipmentPurpose <> 'Return' )
                                        AND TissueIdentifier IS NOT NULL
                                    GROUP BY
                                        a.TissueIdentifier
                                  ) AS ship_1 ON ship_1.TissueIdentifier = import.TissueIdentifier
                  LEFT OUTER JOIN (
                                    SELECT
                                        cco.id
                                       ,MAX(cco.outcome) AS outcome
                  ...
                  LEFT OUTER JOIN SIGHTLIFE.Data.EyeTissue AS et ON import.TissueIdentifier = et.TissueIdentifier
                  LEFT OUTER JOIN (
                                    SELECT
                                        ParentId
                                    FROM
